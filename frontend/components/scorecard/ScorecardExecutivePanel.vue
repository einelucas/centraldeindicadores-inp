...
function formatMeta(indicator: DashboardIndicator) {
  if (!indicator.scoringEnabled) return `Referência ${formatNumber(indicator.meta ?? 0, 0)}`;
...
function indicatorPoints(indicator: DashboardIndicator) {
  return indicator.months.reduce(
    (total, month) => total + (month.passed ? (indicator.peso / 100) * SCORECARD_MONTHLY_POOL : 0),
    0,
  );
}
...
const monthlyPointTotals = computed(() =>
  props.data.monthKeys.map((key) =>
    props.data.indicators.reduce((total, indicator) => {
      ...
      return total + (month?.passed ? (indicator.peso / 100) * SCORECARD_MONTHLY_POOL : 0);
    }, 0),
  ),
);

const atendimentoDisplay = computed(() => `${formatNumber(props.data.atendimentoGeral, 2)}%`);
</script>
...
<template>
  ...
  <div class="scorecard-subcard scorecard-results-card">
    ...
    <div class="scorecard-table-wrap">
      <table class="scorecard-table" :class="{ 'is-admin': variant === 'admin' }">
        <colgroup>
          <col class="scorecard-col-indicator" />
          <col class="scorecard-col-weight" />
          <col class="scorecard-col-target" />
          <col v-for="label in data.monthLabels" :key="label" class="scorecard-col-month" />
        ...
        <thead>
          <tr>
            ...
            <th v-for="label in data.monthLabels" :key="label" class="text-center">{{ label }}</th>
        ...
        <tbody>
          <tr v-for="indicator in data.indicators" :key="indicator.key">
            ...
            <td class="numeric scorecard-brand-emphasis">{{ formatPoints(indicator.peso) }}%</td>
            <td class="numeric scorecard-table-emphasis">{{ formatMeta(indicator) }}</td>
            <td v-for="month in indicator.months" :key="month.key" class="scorecard-month-cell">
              <NuxtLink
                v-if="indicatorRoutes[indicator.key]"
                :to="indicatorRoutes[indicator.key]"
                class="scorecard-month-value"
                :class="month.value === null ? 'is-empty' : month.passed ? 'is-good' : 'is-bad'"
            ...
            <td class="numeric scorecard-table-emphasis">{{ formatValue(indicator, indicator.partial) }}</td>
            <td class="numeric">
              <strong class="scorecard-points">{{ formatPoints(indicatorPoints(indicator)) }}</strong>
              <span class="scorecard-points-maximum">de {{ formatPoints(indicatorMaxPoints(indicator)) }}</span>
            </td>
            <td class="text-center">
              <span v-if="!indicator.scoringEnabled" class="scorecard-status is-dev">Em desenvolvimento</span>
              <span v-else-if="indicator.partialPass === null" class="scorecard-no-data">Sem dados</span>
              <span v-else class="scorecard-status" :class="indicator.partialPass ? 'is-good' : 'is-bad'">
        ...
        <tfoot>
          <tr>
            ...
            <td v-for="(key, index) in data.monthKeys" :key="key" class="text-center">{{ formatPoints(monthlyPointTotals[index] ?? 0) }}</td>
            <td class="numeric">—</td>
            <td class="numeric scorecard-total-points">{{ data.pontosRealizados.toLocaleString("pt-BR") }}</td>
