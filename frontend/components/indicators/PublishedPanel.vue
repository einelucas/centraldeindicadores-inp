...
const props = defineProps<{ module: "rdo" | "idp" | "rnc" | "cinco-s"; title: string }>();
const api = useApi();
...
const monthly = computed(() => {
  ...
  return rows.map(row => ({ label: String(row.label ?? ""), value: typeof row.v === "number" ? row.v : null }));
});
const units = computed(() => {
  ...
  return rows.slice(0, 14).map(row => ({ label: formatUnitLabel(String(row.n ?? "")), value: typeof row.v === "number" ? row.v : null }));
});
...
onBeforeUnmount(() => window.removeEventListener(INDICATOR_DATA_CHANGED_EVENT, load));
</script>
    ...
    <div class="surface">
      ...
      <div v-else class="surface-body stack">
        ...
        <div class="chart-grid">
          <section class="surface chart-card"><h3>Evolução mensal</h3><p>Resultado publicado ao longo do período.</p><LineChart :points="monthly" :target="target" :suffix="module === 'rnc' ? '' : '%'" /></section>
          <section class="surface chart-card"><h3>Resultado por unidade</h3><p>Comparativo das unidades incluídas na publicação.</p><BarChart :points="units" :target="target" :suffix="module === 'rnc' ? '' : '%'" /></section>
        </div>
