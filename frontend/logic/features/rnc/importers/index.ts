 ...
 * Migrado de handleRncFiles() + combineAndRenderRnc() do sistema original.
 * Usa detecção de cabeçalho e dedup de linha 100% idêntica entre arquivos.
 */

import { readWorkbookMatrix } from "~/logic/importers/shared/workbook-reader";
import { parseWithHeaderDetection } from "~/logic/importers/shared/header-detection";
import { parseFlexDate } from "~/logic/lib/dates";
import { parseBRNumberOrNull } from "~/logic/lib/currency";
import { normalizeRncUnitCode } from "~/logic/features/rnc/utils/units";
import { REQUIRED_RNC_COLS, type RncNormalizedRecord } from "~/logic/features/rnc/types";
...
export async function parseRncFile(file: File): Promise<RncFileParseResult> {
  const result: RncFileParseResult = { fileName: file.name, rows: null, count: 0, error: null };
  try {
...
/** Remove linhas 100% idênticas entre arquivos. */
...
export function toRncRecord(row: Record<string, unknown>): RncNormalizedRecord | null {
  const dataCriacao = parseFlexDate(row["data_de_criação"]);
  if (!dataCriacao) return null;

  const solRaw = row["data_de_solução"];
  const dataSolucao =
    solRaw === null || solRaw === undefined || solRaw === "" ? null : parseFlexDate(solRaw);

  const tRaw = row["tempo_de_tratativa"];
  const tempoTratativa =
    tRaw === null || tRaw === undefined || tRaw === "" || String(tRaw).trim().toUpperCase() === "NAN"
      ? null
...
export async function importRncFiles(files: File[]): Promise<{
  ...
  const records = rows.map(toRncRecord).filter((r): r is RncNormalizedRecord => r !== null);
