 ...
 * Migrado de handleRdoFiles() + combineAndRenderRdo() do sistema original.
 *
 * Fluxo:
 *   1. lê cada arquivo (Excel/CSV) -> linhas brutas
 *   2. normaliza headers
 *   3. valida colunas obrigatórias
 *   4. deduplica linhas 100% idênticas entre arquivos
 *   5. converte para RdoNormalizedRecord (com data parseada e ano/mês)
 */

import { readWorkbookFile } from "~/logic/importers/shared/workbook-reader";
import { normalizeRows, nullIfEmpty } from "~/logic/lib/normalization";
import { parseFlexDate } from "~/logic/lib/dates";
import { REQUIRED_RDO_COLS, type RdoNormalizedRecord } from "~/logic/features/rdo/types";
...
export async function parseRdoFile(file: File): Promise<RdoFileParseResult> {
  const result: RdoFileParseResult = { fileName: file.name, rows: null, count: 0, error: null };
  try {
 ...
 * relatorio_id sozinho repete legitimamente, então só uma linha inteira
 * idêntica conta como duplicata real.
 */
...
export function toRdoRecord(row: Record<string, unknown>): RdoNormalizedRecord | null {
...
export async function importRdoFiles(
  files: File[],
): Promise<{ records: RdoNormalizedRecord[]; perFile: RdoFileParseResult[]; duplicates: number }> {
  ...
  const records = rows.map(toRdoRecord).filter((r): r is RdoNormalizedRecord => r !== null);
