...
import type { FiveSArea, FiveSNormalizedRecord } from "~/logic/features/cinco-s/types";
import { normalizeFiveSUnitCode } from "~/logic/features/cinco-s/utils/units";
...
export async function parseFiveSFile(file: File): Promise<FiveSFileParseResult> {
  const result: FiveSFileParseResult = { fileName: file.name, records: [], error: null };
  try {
    ...
    if (!result.records.length) {
      result.error = 'Não encontrei nenhuma aba com o padrão "Meta" + data + Divisão/Área/Nota.';
    }
  } catch (error) {
