...
export function parseDisciplineLine(text: string, disciplineName: string) {
  ...
  const pattern = new RegExp(`${escaped}\\s+([\\d.,]+)%\\s+([\\d.,]+)%\\s+[+-]?[\\d.,]+%`, "i");
...
export function parseRsoReferenceMetadata(text: string): RsoReferenceMetadata {
  ...
  const full = normalized.match(/m[eê]s\s*ref\.?\s*:\s*(\d{1,2})[./-](\d{1,2})[./-](\d{2,4})/i);
  if (full) {
    const month = Number(full[2]);
    const year = normalizeYear(Number(full[3]));
    if (isoDate(year, month, 1)) {
      return { referenceYear: year, referenceMonth: month, originalText: full[0] ?? null };
    }
  }

  const numeric = normalized.match(/m[eê]s\s*ref\.?\s*:\s*(\d{1,2})[./-](\d{2,4})/i);
  if (numeric) {
    const month = Number(numeric[1]);
    const year = normalizeYear(Number(numeric[2]));
    if (isoDate(year, month, 1)) {
      return { referenceYear: year, referenceMonth: month, originalText: numeric[0] ?? null };
    }
  }
  ...
  if (named) {
    ...
    if (month && isoDate(year, month, 1)) {
      return { referenceYear: year, referenceMonth: month, originalText: named[0] ?? null };
    }
  }
...
export function parseRsoPeriodMetadata(text: string): RsoPeriodMetadata {
  ...
  const emission = normalized.match(/emiss[aã]o\s*:\s*(\d{1,2})[./-](\d{1,2})[./-](\d{2,4})/i);
...
export async function parseRsoPdf(file: File): Promise<IdpNormalizedRecord> {
  ...
  const areas: string[] = [];
  const discData: IdpRsoDisciplineData = Object.fromEntries(IDP_DISC_NAMES.map((name) => [name, []]));
...
export async function importIdpFiles(
  files: File[],
): Promise<{ records: IdpNormalizedRecord[]; perFile: IdpFileParseResult[] }> {
