...
import type { IdpResult } from "~/logic/features/idp/types";
import { MONTH_NAMES_FULL } from "~/logic/lib/dates";
...
/** Gera e salva o PDF de indicadores do IDP (tabelas por unidade e por disciplina). */
export function exportIdpPdf(result: IdpResult): void {
  const pdf = new jsPDF({ unit: "pt", format: "a4", orientation: "landscape" });
  const competence = `${MONTH_NAMES_FULL[result.selectedMonth - 1] ?? result.selectedMonth}/${result.selectedYear}`;
  ...
  autoTable(pdf, {
    startY: finalY + 34,
    head: [["Disciplina", "Unidades", "Prev. médio", "Real médio", "Aderência"]],
    body: result.disciplineRows.map((row) => [
      row.disciplina,
      String(row.unitGroups.length),
